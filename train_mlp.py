...
start = time.time()
model_pl = MLPClassifier(hidden_layer_sizes=(256, 128), max_iter=100, random_state=42, early_stopping=True)
...
X_train_cfo = np.column_stack([X_train_scaled, y_train_pl])
X_test_cfo = np.column_stack([X_test_scaled, le_pl.transform(y_pred_pl)])

start = time.time()
model_cfo = MLPClassifier(hidden_layer_sizes=(256, 128), max_iter=100, random_state=42, early_stopping=True)
...
model_bundle = {'model_pl': model_pl, 'model_cfo': model_cfo, 'le_pl': le_pl, 'le_cfo': le_cfo, 'scaler': scaler}
with open('model_mlp.pkl', 'wb') as f:
    pickle.dump(model_bundle, f)

results = {'model': 'MLP', 'pl_accuracy': float(acc_pl), 'cfo_accuracy': float(acc_cfo), 'pl_time': float(pl_time), 'cfo_time': float(cfo_time)}
with open('results_mlp.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSaved: model_mlp.pkl, results_mlp.json")
