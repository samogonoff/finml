...
start = time.time()
model_pl = lgb.LGBMClassifier(n_estimators=100, max_depth=10, learning_rate=0.1, random_state=42, n_jobs=-1, verbose=-1)
...
start = time.time()
model_cfo = lgb.LGBMClassifier(n_estimators=100, max_depth=10, learning_rate=0.1, random_state=42, n_jobs=-1, verbose=-1)
...
results = {'model': 'LightGBM', 'pl_accuracy': float(acc_pl), 'cfo_accuracy': float(acc_cfo), 'pl_time': float(pl_time), 'cfo_time': float(cfo_time)}
with open('results_lightgbm.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSaved: model_lightgbm.pkl, results_lightgbm.json")
