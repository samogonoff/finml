...
start = time.time()
model_pl = xgb.XGBClassifier(n_estimators=50, max_depth=8, learning_rate=0.1, random_state=42, n_jobs=-1, eval_metric='mlogloss')
...
results = {'model': 'XGBoost', 'pl_accuracy': float(acc_pl), 'cfo_accuracy': float(acc_cfo), 'pl_time': float(pl_time), 'cfo_time': float(cfo_time)}
with open('results_xgboost.json', 'w') as f:
    json.dump(results, f, indent=2)

print(f"\nSaved: model_xgboost.pkl, results_xgboost.json")
